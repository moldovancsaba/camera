...
| **Grid outer size** | On `/slideshow-layout/...` the **entire** rigid grid **always** scales to fit the viewport (contain): `layoutGridStageDimensions(..., 'fit')` plus **`aspect-ratio: (cols×16)/(rows×9)`** (`layoutGridAspectRatioCss`) and max constraints so the videowall is never cropped. `layoutGridStageDimensions(..., 'fill')` exists in code but is **not** used for this public surface. **Per-region `objectFit`** still controls cover/contain **inside** a tile. |
