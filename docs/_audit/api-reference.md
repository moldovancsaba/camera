...
| GET /api/internal/savetheworld/events | assertInternalSavetheworldSecret; rate limit INTERNAL_READ | `?partnerId` filter; `?eventId=<eventId or Mongo _id>` exact lookup (v12.3.33) | `{events[{eventId, name, partnerId, partnerName, eventDate, isActive, mongoId, captureUrl}]}` — unfiltered list capped at 200 rows, newest eventDate first | none |
