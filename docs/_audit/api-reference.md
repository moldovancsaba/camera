...
| GET /api/internal/savetheworld/pledges | assertInternalSavetheworldSecret; rate limit INTERNAL_READ | `?eventId=<Mongo _id or event UUID>&limit (default 12, max 60)` (no eventId → empty list) | `{pledges[{pledgeId, imageUrl, name, createdAt}]}` — only `isShareVisible` originals with a `finalImageUrl`; never email/userInfo | none |
