...
| GET /api/internal/savetheworld/events | assertInternalSavetheworldSecret; rate limit INTERNAL_READ | `?partnerId` filter; `?eventId=<eventId or Mongo _id>` exact lookup (v12.3.35) | `{events[{eventId, name, partnerId, partnerName, eventDate, isActive, mongoId, captureUrl}]}` — unfiltered list capped at 200 rows, newest eventDate first | none |
...
| GET /api/internal/savetheworld/pledges | assertInternalSavetheworldSecret; rate limit INTERNAL_READ | `?eventId=<Mongo _id or event UUID>&limit (default 12, max 60)` (no eventId → empty list; either id form resolves the event and matches submissions on all of its identifiers since v12.3.35) | `{pledges[{pledgeId, imageUrl, name, createdAt}], total}` — only `isShareVisible` originals with a `finalImageUrl`; `total` counts them regardless of `limit` (v12.3.35); never email/userInfo | none |
