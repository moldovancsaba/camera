...
- ~~**Session cookies are neither encrypted nor signed** despite
  ...
  FIXED 2026-09-08 (v12.3.32, camera#122): the plain cookie is HMAC-signed
...
- ~~**W8** lib/tryon/completion.ts:244 error says "direct i.ibb.co" but the
  ...
  RESOLVED (12.3.32, camera#126): the message now names both accepted host
...
- Contradicting comments: session.ts:5 ("encrypted"), :8 ("extends on each
  ...
  `void request`). ~~completion.ts:244 (see W8)~~ RESOLVED (12.3.32, see W8).
  ~~messmassClient.ts:8-11 (claims a `source!=='messmass'` guard that exists
  only on the update path)~~ RESOLVED (12.3.32, camera#126): the header now
...
- ~~`app/admin/tryon-results` + `tryon-suits` pages are re-exported by
  ...
  remain at identity/analytics pages.~~ RESOLVED (12.3.32, camera#125): the
...
- ~~Stale planning docs: docs/GDS_3_4_3_*, GDS_3_5_ADOPTION_PLAN, ISSUE_AUDIT_2026-06-30,
  ...
  REMOVED (12.3.32, camera#125): all seven deleted; the links from README,
