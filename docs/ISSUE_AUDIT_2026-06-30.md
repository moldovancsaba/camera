...
| Issue | Fix | Verified |
|---|---|---|
| **#75 residual** | The two `window.confirm` calls in `components/admin/TryOnQueueTable.tsx` (rerun / reapply) now use `useGdsConfirm().confirm()` from `@doneisbetter/gds-core/client`, matching the house idiom. | type-check + lint clean |
...
Still open after this branch: **#83** (observability sink — needs a Sentry-vs-Vercel-logging decision), **#74/#76/#77** (remaining GDS UI migrations), **#82** (custom lint rule), **#78** (rewrite around the post-CI enforcement model), and the board reconciliation itself (closing the 13 verified-done issues).
